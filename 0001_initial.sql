...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    action TEXT CHECK(action IN ('Keep', 'Delegate', 'Automate', 'Eliminate')),
    notes TEXT,
    focus_score REAL,
    fear_rating INTEGER CHECK(fear_rating >= 1 AND fear_rating <= 10),
...
CREATE TABLE IF NOT EXISTS task_templates (
    ...
    task_name TEXT NOT NULL,
    default_intensity INTEGER CHECK(default_intensity >= 1 AND default_intensity <= 10),
    default_roi INTEGER CHECK(default_roi >= 1 AND default_roi <= 10),
    default_burn INTEGER CHECK(default_burn >= 1 AND default_burn <= 10),
...
-- Week planner
CREATE TABLE IF NOT EXISTS week_planner (
    id TEXT PRIMARY KEY DEFAULT (lower(hex(randomblob(16)))),
    user_id TEXT NOT NULL,
    week_start DATE NOT NULL,
    day_of_week INTEGER CHECK(day_of_week >= 0 AND day_of_week <= 6),
    planned_frog TEXT,
    planned_intensity INTEGER,
...
CREATE INDEX idx_tasks_is_frog ON tasks(user_id, is_frog);
CREATE INDEX idx_tasks_is_pr ON tasks(user_id, is_pr);
CREATE INDEX idx_tasks_intensity ON tasks(user_id, intensity);
CREATE INDEX idx_tasks_roi ON tasks(user_id, roi);
CREATE INDEX idx_streaks_user ON streaks(user_id, streak_type);
CREATE INDEX idx_badges_user ON badges(user_id);
CREATE INDEX idx_week_planner_user ON week_planner(user_id, week_start);

-- Insert default scoring guide data
INSERT INTO scoring_guide (metric, score, description, examples) VALUES
-- Intensity scores
...
INSERT INTO task_templates (category, task_name, default_intensity, default_roi, default_burn, is_global) VALUES
('Deep Work', 'Strategic Planning', 8, 9, 7, 1),
('Deep Work', 'Complex Problem Solving', 9, 8, 8, 1),
('Deep Work', 'Creative Design', 7, 7, 6, 1),
('Deep Work', 'Writing/Documentation', 6, 7, 5, 1),
('Meetings', 'Client Meeting', 6, 8, 5, 1),
('Meetings', 'Team Standup', 3, 5, 2, 1),
('Meetings', '1-on-1', 5, 6, 4, 1),
('Meetings', 'Strategy Session', 8, 9, 7, 1),
('Admin', 'Email Processing', 2, 3, 2, 1),
('Admin', 'Expense Reports', 3, 2, 3, 1),
('Admin', 'Calendar Management', 2, 4, 2, 1),
('Learning', 'Course/Training', 5, 8, 4, 1),
('Learning', 'Reading/Research', 4, 7, 3, 1),
('Sales', 'Cold Outreach', 7, 8, 7, 1),
('Sales', 'Proposal Writing', 7, 9, 6, 1),
('Sales', 'Follow-up', 4, 6, 3, 1);
